...
function Profile() {
  ...
  useEffect(() => {
    ...
    if (account) {
      fetch(`https://financefor.fun:3000/api/courses`)
        .then((res) => res.json())
        .then((list) => {
          if (list) {
            setCourses(list);
  ...
  useEffect(() => {
    ...
    if (account) {
      ...
      fetch(`https://financefor.fun:3000/api/users/${account}/tokens`)
        .then((res) => {
          return res.json();
        })
        .then((amount) => {
  ...
  useEffect(() => {
    ...
    if (account) {
      ...
      fetch(`https://financefor.fun:3000/api/courses/evaluations`)
        .then((res) => res.json())
        .then((list) => {
          if (list) {
